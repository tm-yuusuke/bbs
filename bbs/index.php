...
require('dbconnect.php');
  ...
  if(isset($_SESSION['id']) && $_SESSION['time'] + 3600 > time()){
    $_SESSION['time'] = time();//アクションを起こした時間を更新して自動ログアウトしないようにする
  ...
  if(!empty($_POST)){//$_POSTが空ではないとき。つまり投稿するボタンが押されたとき。
    if($_POST['message'] !==''){//テキストボックスエリアが空白で無いとき。
      ...
      header('Location: index.php');//この処理をしないと$_POSTにメッセージが残ってるので、更新を押すとテキストエリアが空白なのに同じメッセージが重複して登録される。
      exit();
    }
  }
//ページネーションの処理
...
$page = max($page,1);//max関数。$pageと1を比べ、1の方が大きい時は1を入れる

$counts = $db->query('SELECT COUNT(*) AS cnt FROM posts');
$cnt = $counts->fetch();
$maxPage = ceil($cnt['cnt'] / 5);//dbからcountした件数を5で割り少数は切り上げて最大ページ数にする。例えば10件なら10÷5=2ページ13件なら13÷5=2.6切り上げて3ページ
$page = min($page, $maxPage);//maxpageと入力されたページ数を比較して、入力が大きかったらmaxpageを入れる
$start = ($page - 1) * 5;//１ページ目だと(1-1)×5で0から5件。2ページ目だと(2-1)×5=5で5から10件を取得

$posts = $db->prepare('SELECT m.name, m.picture, p.* FROM members m, posts p WHERE m.id=p.member_id ORDER BY p.created DESC LIMIT ?,5');
$posts->bindParam(1, $start, PDO::PARAM_INT);//LIMITの?は数字が入る必要がある。executeにすると文字として入るので、bindParamで数字に縛る
$posts->execute();
//ページネーションの処理
